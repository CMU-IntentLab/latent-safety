...
#from LCRL.policy.modelfree.BasePolicy_Annealing_Reach_Avoid import BasePolicy_Annealing_Reach_Avoid as BasePolicy # correct
from LCRL.policy.modelfree.BasePolicy_Annealing_Avoid import BasePolicy_Annealing_Avoid as BasePolicy # correct


class reach_avoid_game_DDPGPolicy_annealing(BasePolicy):
    ...
    def train(self, mode: bool = True) -> "reach_avoid_game_DDPGPolicy_annealing":
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        return Batch(act=torch.cat((actions1, actions2), dim=1), 
                    state=hidden1)
    ...
    def learn(self, batch: Batch, **kwargs: Any) -> Dict[str, float]:
        ...
        # update player 2's actor
        # note that for self.act2, we fix the action of player 1
        for _ in range(self.actor_gradient_steps):
            actor2_loss = self.critic(batch.obs, self(batch, model="actor").act).mean()
