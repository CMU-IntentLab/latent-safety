...
if __name__ == "__main__":
    ...
    iters = []
    train_iter = 100000
    print(len(expert_loader), len(expert_loader_eval), len(expert_loader_imagine))

    for i in tqdm(range(train_iter), desc="Training", unit="iter"):
        if i % len(expert_loader) == 0:
            expert_loader = iter(DataLoader(expert_data, batch_size=BS, shuffle=True))
        if i % 500: #len(expert_loader_eval) == 0:
            expert_loader_eval = iter(DataLoader(expert_data_eval, batch_size=BS, shuffle=True))
        ...
        if (i) % 500 == 0:
            ...
            with torch.no_grad():
                ...
                print(eval_data['failure'][[0]].squeeze().shape, gt_fail.shape)
            ...
            torch.save(transition.state_dict(), f'checkpoints/classifier_iter{i}.pth')
